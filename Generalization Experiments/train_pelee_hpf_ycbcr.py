...
train_imgs = ImageFolder(f"./Datafolder/Add/train(Add_10)/",
                         transform=transforms.Compose([
                                 transforms.ToTensor()]
))
test_imgs = ImageFolder(f"./Datafolder/LSUN/test(sheep_PGGAN)/",
                        transform=transforms.Compose([
                                transforms.ToTensor()]
))
...
def train_net(net, train_loader, test_loader,
    ...
    torch.save(net.state_dict(), f"./result/Add/epoch(Add_10(YCbCr)_pelee_hpf)_49.pth")

def Evaluate_Networks(Net):
    save_path = "./result/Add/"
    # data
    Net.load_state_dict(torch.load(save_path + "epoch(Add_10(YCbCr)_pelee_hpf)_49.pth"), strict=False)
